...
int coefficent(double *x, size_t row, size_t col, double *y, double *risultato)
{
    ...
    if (!vander_mat || !holder || !vander_trasposta || !denominatore || !numeratore)
    {
        free(vander_mat);
        free(holder);
    ...
    if (!gauss_inversion(holder, col, col, denominatore))
    {
        free(vander_mat);
        free(holder);
        free(vander_trasposta);
        free(denominatore);
        free(numeratore);
        return 0; // Errore nell'inversione
    }
...
int main()
{
    ...
    if (!coefficent(x, dim_x, ordine, y, coefficenti))
    {
        fprintf(stderr, "Errore nel calcolo dei coefficienti.\n");
        free(coefficenti);
        return 1;
    }

    printf("Coefficienti del polinomio:\n");
