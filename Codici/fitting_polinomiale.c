...
void coefficent (double *x, size_t row, size_t col, double *y, double *risultato)
{
    ...
    double *a = (double*)malloc(col * sizeof(double));
    ...
    prod_matrice(vander_mat, row, col, vander_mat, col, holder);
    trasposta(vander_mat, row, col, vander_trasposta);
    gauss_inversion(holder, col, col, denominatore);
    prod_matrice(vander_trasposta, row, col, y, 1, numeratore);
    prod_matrice(denominatore, col, col, numeratore, 1, a);
}
...
int main()
{
 ...
 double *mat_vander = (double*)malloc(dim_x * ordine * sizeof(double));
 double *coefficenti = (double*)malloc(ordine * sizeof(double));

 coefficent(x, dim_x, ordine, y, coefficenti);
 stampa_matrice(coefficenti, dim_x, 1);
